...
class ColorSensor extends Component {
	...
	swapMode (e, i, item) {
		this.props.dispatch(setSensorMode(this.props.path, item.payload))
	}
	colorValue () {
	...
	componentDidMount () {
		this.interval = setInterval(() => this.props.dispatch(getSensorData(this.props.path, this.props.port)), 1000)
	}
